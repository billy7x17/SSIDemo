...
<script type="text/JavaScript">
	$(function() {
		...
		$("#alarmOID").formValidator({
			...
			empty : {
				leftempty : false,
				rightempty : false,
				emptyerror : '<s:text name="validator.alarmOID.emptyerror" />'
			},
				...
				.inputValidator(
						{
							...
							empty : {
								leftempty : false,
								rightempty : false,
								emptyerror : '<s:text name="validator.alarmTitle.emptyerror" />'
							},
		...
		$("#alarmContent").formValidator({
			...
			min : 0,
			max : 500,
			onerrormax : '<s:text name="validator.alarmContent.onerrormax" />'
		});
		...
		$("#alarmImpact").formValidator({
			...
			min : 0,
			max : 512,
			onerrormax : '<s:text name="threshold.js.512char" />'
		});
		...
		//告警标题，增加title属性
		$('option', '#alarmTitle').attr('title', function() {
	...
	function callbackFun(data) {
		...
		$("#alarmTitle").empty();
		$("#alarmTitle").append(
				"<option value='null'>" + '<s:text name="lable.select" />'
						+ "</option>");
		for ( var key in titleLi) {
			if (typeof (titleLi[key].tcId) != "undefined") {
				$("#alarmTitle").append(
						"<option value='"+titleLi[key].tcId + "' title='" + titleLi[key].tcTitle +"'>"
								+ titleLi[key].tcTitle + "</option>");
			}
		}
	}
</script>
...
<div class="rightDisplayFwSj">
	<div class="formGroup">
		<form id="mainForm" method="post"
			action="alarmrulesEdit.action?alarmrules.rowID=<s:property value='alarmrules.rowID'/>">
			<div class="formLabel">
				<s:text name="common.lable.baseInfo" />
				<s:text name="common.lable.line" />
			</div>
			<s:hidden id="alarmIdentityID" name="alarmrules.alarmIdentityID" />
			<table class="addTable" align="center" cellpadding="0"
				...
				<tr>
					...
					<td class="tdInput" valign="top"><s:select
							list="alarmTitleList" id="alarmTitle" listKey="alarmTitle"
							listValue="alarmTitleContent" name="alarmrules.alarmTitle"
							cssClass="newSelect" headerKey=""
							headerValue="%{getText('lable.select')}">
						</s:select></td>
				...
				<tr>
					...
					<td class="tdInput" valign="top"><input type="text"
						...
						value="<s:property value="alarmrules.alarmOID"/>" class="input" />
					</td>
					...
					<td class="tdInput" valign="top">
						<div>
							...
							<textarea name="alarmrules.alarmContent" id="alarmContent"
								...
								onkeyup="validateTextarea();" style="margin-bottom: 15px"><s:property value="alarmrules.alarmContent"/></textarea>
						</div>
					</td>
					...
					<td class="tdInput" valign="top">
						<div>
							...
							<textarea name="alarmrules.alarmImpact" id="alarmImpact"
								...
								onkeyup="validateTextareaImpact();"><s:property value="alarmrules.alarmImpact"/></textarea>
						</div>
					</td>
