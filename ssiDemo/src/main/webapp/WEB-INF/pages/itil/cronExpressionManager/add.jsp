...
window.onload = function(){
    ...
    $("#TASK_TYPE").formValidator({
      ...
      $("#BEAN_ID")
      .formValidator({
       onshow :'<s:text name="cronExpression.manager.validator.beanID.onfocus" />'
           , onfocus : '<s:text name="cronExpression.manager.validator.beanID.onfocus" />'
           , oncorrect : '<s:text name="common.validator.oncorrect"/>'
      })
      .functionValidator({
    	  fun:function(val, elem){
    		  if(val == "" || val == null){
				return '<s:text name="cronExpression.manager.validator.beanID.onerror" />';
    		  }
			  return true;
    	  }
      });
      ...
      .inputValidator({
                 min : 0
                   , onerror: '<s:text name="cronExpression.manager.validator.beanID.onerror" />'
      });//end validator
    ...
    $('#EXCUTE_TIME').bind('focus',function(){
    	...
    	if(src == ""){
    		var url = "<%=request.getContextPath()%>/cronExpressionDate.action";
            var value = $('#EXCUTE_TIME').val();
            if(value != null && value != ''){
            	url = url + "?domain.cronExpression=" + value;
            }
    ...
    $("#SS_ID").bind("change", function(){getJobBySSID();});
    getJobBySSID();
};

function setExecuteTime(expression){
    $('#EXCUTE_TIME').val(expression);
    $("#desc_tr").removeAttr("style");
    $("#cronExpressionDesc").html(translateCronExpression(expression) + "<br/><br/>");
}
...
function validate(){
    ...
    if(formCheck){
        $("#add_form").attr("action","cronExpressionAdd.action");
        $("#add_form").submit();
    }else{
        return false;
    }
...
function getJobBySSID(){
	var ssID = $("#SS_ID").val();
	$.ajax({
        type : "post",
        url : "getJobsBySSID.action",
        async : true,
        data : "SSID=" + ssID,
        dataType : "text",
        success : function(data){
            ...
            if(data == "error"){
            	...
            	for(var i = 0; i<dataArr.length;i++){
            		$("<option value=\'"+dataArr[i].beanID+"\'>"+dataArr[i].jobName+"</option>").appendTo("#BEAN_ID");
            	}
            }
        },
...
<div id="dialog" style="padding:0;" align="center" title="<s:text name="cronExpression.dialog.title" />">
...
<div class="rightDisplayFwSj">
    <form id="add_form" method="post" action="#" >
      <s:hidden name="domain.taskID" id="TASK_ID"></s:hidden>
      <s:hidden id="TASK_TYPE" name="domain.taskType" value="SPRING"></s:hidden>
      <table class="addTable" align="center" cellpadding="0" cellspacing="0" >
	    ...
	    <tr>
	        ...
	        <td class="tdInput" valign="top">
	            <s:select id="SS_ID" name="domain.ssID"
	                headerKey="" headerValue="%{getText('cronExpression.ssID.default')}"
	                list="ssList" listKey="ssID" listValue="ssText" cssClass="newSelect"/>
	        </td>
	        <td class="tdVad" valign="top">
	            <div id="SS_IDTip"></div>
	        </td>
	    ...
	    <tr>
			...
			<td class="tdInput" valign="top">
				<!-- <s:textfield id="BEAN_ID" name="domain.beanID" maxlength="64" cssClass="input"></s:textfield> -->
				<select id="BEAN_ID" name="domain.beanID" class="newSelect"></select>
			</td>
			<td class="tdVad" valign="top">
...
<div class="serverEventButton">
    <ul class="rightToolButtonjhjFwSj">
        <li><a href="#" class="buttonFwSj" onclick="validate()"><s:text name="common.button.add" /></a></li>
        <li><a href="cronExpressionList.action" class="buttonFwSj" ><s:text name="common.button.cancel" /></a></li>
    </ul>
</div>
