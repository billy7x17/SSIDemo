...
function getTab() {
	$.ajax({
        url: "getAlreadyConfTabIpc.action",
        type: "POST",
        ...
        success:function(tab) {//请求Tab页信息
			...
			for(var i = 0 ; i < result.length ; i++) {
				...
				if (i == 0) {
					//默认选中第一个Tab
	        		$("#task1 .btn-group").append('<button id="'+result[i]+'ReportButton" style="padding-left:20px;" class="small-btn btn-grey" onclick="change('+"'"+result[i]+'Report'+"'"+');return false;"><span>'+result[i]+'</span></button>');
				}else {
					//其它Tab不选中
	        		$("#task1 .btn-group").append('<button id="'+result[i]+'ReportButton" style="padding-left:20px;"  class="small-btn btn-eee" onclick="change('+"'"+result[i]+'Report'+"'"+');return false;"><span>'+result[i]+'</span></button>');
				}
				//拼所有折线图和表格的外框
 		       	$("#task1").append('<div id="'+result[i]+'Report" class="report" style="height: 50%"><div id="pmres'+result[i]+'" class="reportdiv"></div></div>');
				//默认展示第一个Tab下的div
				if (i==0) {
...
function getTabIndex(){
	$.ajax({
        url: "getAlreadyConfTabIndexIPC.action",
        type: "POST",
        data:{deviceIp:""+'<s:property value="#parameters.deviceIp"/>',typeId:'<s:property value="#parameters.typeId"/>'},
        dataType: "json",
...
function setTableHeight(){
	$("#task1").height($(window).height()-$(".rightToolbar").height()-15);
	$(window).resize(function(){
		$("#task1").height($(window).height()-$(".rightToolbar").height()-15-10);
	});
}
		...
		for (var one in tabIndexArray)
		{
			...
			if (result[t]==tabIndexArray[one]["indexGroup"] && tabIndexArray[one]["ifShowLine"] == "1")
			{
				...
				for (var ii in temp)
				{
					...
					if(tabIndexArray[one]["indexGroup"]==temp[ii]["mibName"]||temp[ii]["mibName"]==''){
			...
			for(var ii in temp){
				...
				if (seriesNameUnit==nameandunit && temp[ii]["ifShowLine"] == "1"){
					if(isNaN(parseFloat(temp[ii]["tagetValue"]))){
						eval('chart'+bb).series[aa].addPoint([temp[ii]["showTime"].time,null],true,true);
					}else{
						eval('chart'+bb).series[aa].addPoint([temp[ii]["showTime"].time,Math.round(parseFloat(temp[ii]["tagetValue"])*100)/100],true,false);
					}
				}
			}
		}
...
function getIncreaseData(){
	$.ajax({
		url: "searchIPCResourceIncrease.action",
        type: "POST",
        data:{deviceIp:""+'<s:property value="#parameters.deviceIp"/>',typeId:'<s:property value="#parameters.typeId"/>'},
        dataType: "text",
        success:function(pmList) {
...
.reportdiv {margin-left: 10px ; margin-top: 2px; width: 98%; height: 400px;}


.ou {border-top:1px solid #666; line-height:26px;}
.ed {background:#f2f2f2; border-top:1px solid #666; line-height:26px;}
.stripe_tb tr.alt {border-top:1px solid #666; line-height:26px;} /*这行将给所有偶数行加上背景色*/
.stripe_tb tr.ou {background:#f2f2f2; border-top:1px solid #666; line-height:26px;} /*这行将给所有偶数行加上背景色*/
...
function timeChange(obj) {
	...
	getTimeDate(optionTime,'<s:property value="#parameters.deviceIp"/>','<s:property value="#parameters.typeId"/>');
}
function getTimeDate(optionTime,ip,typeId) {
	$.ajax({
		url: "historyResourceIpc.action",
        type: "POST",
        data:{optionTime:""+optionTime,deviceIp:ip,typeId:typeId},
        dataType: "text",
